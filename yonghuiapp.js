/******************************
[rewrite_local]
^https:\/\/api.yonghuivip.com\/web\/coupon\/signreward\/sign url script-request-body https://raw.githubusercontent.com/cddchen/scripts/main/yonghuiapp.js

[mitm] 
hostname = api.yonghuivip.com

*******************************/
const $ = new Env('永辉生活APP')

const envSplitor = ['`']
const ckNames = ['YONGHUIAPP']
const MAX_THREAD = 3
  ...
  async sign() {
    try {
      const headers = {
        ...
        "User-Agent": 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 YhStore/9.1.5(client/phone; iOS 16.1; iPhone13,1)',
        Origin: 'yhwebcachehttps://m.yonghuivip.com',
        'X-YH-Context': 'origin=h5&morse=1',
        'X-YH-Biz-Params': "xdotdy=,)&gib=-$+0)+)!_(&gvo=+$0$)_*')"
      }
      ...
      if (code == 0) {
        // $.notifyStr.push(`签到成功：${result.message}，获得${result.data.integral}`)
        this.log(`签到成功：${result.message}，获得${result.data.signcredit}`)
      } else {
        // $.notifyStr.push(`获取token失败：${result.message}`)
        this.log(`获取token失败：${result?.message}`)
      }
    } catch(e) {
      this.log(`获取token出错，${e}`)
      // $.notifyStr.push(`获取token出错，${e}`)
    } finally {
