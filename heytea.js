/******************************
[rewrite_local]
^https:\/\/vip.heytea.com\/api\/service-member\/vip\/task\/award\/114$ url script-request-body https://raw.githubusercontent.com/cddchen/scripts/main/heytea.js
  ...
  async sign() {
    try {
      const headers = {
        ...
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_0_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.33(0x18002129) NetType/4G Language/en',
        'referer': 'https://servicewechat.com/wx696a42df4f2456d3/1016/page-frame.html'
      }
      ...
      if (message === 'success') {
        let score = result?.data?.score ?? 'Unknown'
        this.log(`签到成功，获得${score}积分`)
        return Promise.resolve(`签到成功，获得${score}积分`)
      } else {
        // $.notifyStr.push(`获取token失败：${result?.errorMessage}`)
        this.log(`签到失败：${message}`)
        return Promise.resolve(`签到失败：${message}`)
      }
    } catch (e) {
      this.log(`获取token出错，${e}`)
      return Promise.reject(e)
    }
  }

  async userTask() {
    this.message.push(`***********${this.name}************`)
    
    await this.sign().then(m => {
      this.message.push(`签到完成，结果为：${m}`);
    }).catch(e => {
      this.message.push(`签到异常，结果为：${e}`)
    })
  }
}
