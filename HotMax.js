...
class UserClass {
  ...
  async sign() {
    try {
      const headers = {
        Host: 'openapi-gateway.hotmaxx.cn',
        Connection: 'keep-alive',
        'Content-Type': 'application/json',
        'Authorization': this.session['Authorization'],
        'Accept-Encoding': 'gzip,compress,br,deflate',
        'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.31(0x18001f37) NetType/WIFI Language/en',
        Referer: 'https://servicewechat.com/wx35e6b745bcb73b2c/117/page-frame.html'
      }

      const url = {
        url: 'https://openapi-gateway.hotmaxx.cn/member/sign/signIn',
        headers: headers,
        body: JSON.stringify(this.session['body'])
      }
      this.log(JSON.stringify(url))
      let { result } = await $.post(url)
      this.log(JSON.stringify(result))
      let code = result?.code
      if (code == 200 && result?.data.growValue) {
        return Promise.resolve(`签到成功：${result?.msg}，获得${result?.data.growValue}积分`)
      } else {
  ...
  async share(userId) {
    try {
      ...
      // const body = {
      //   "sharePage": "index",
      //   "pointUserId":"1651981984973725694",
      //   "userId":"1651981984973725694",
      //   "shareUserId":"1644308113971019776"
      // }
      
      const body = {
        "sharePage": "index",
        "pointUserId": `1651981984973725${userId}`,
        "userId": `1651981984973725${userId}`,
        "shareUserId": this.session['shareUserId']
      }
      const url = {
        url: 'https://openapi-gateway.hotmaxx.cn/member/share/saveShareRecord',
        headers: headers,
      ...
      let code = result?.code
      if (code == 200) {
        return Promise.resolve(`分享成功：${result?.msg}`)
      } else {
        return Promise.reject(`分享失败：${result?.msg}`)
      }
    } catch (e) {
      return Promise.reject(`error：${e.message}`)
    }
  ...
  async userTask() {
    for (let userid = 0; userid < 10; userid++) {
      await this.share(694-userid).then(m => {
        this.log(`【${userid+1}/10】${m}`)
      }).catch(e => {
        this.log(`【${userid+1}/10】${e}`)
      })
    }
    await this.sign().then(m => {
      this.message.push(m)
    }).catch(e => {
      this.message.push(e)
    })
