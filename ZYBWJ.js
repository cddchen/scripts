...
if (isGetCookie) {
	...
	!(async () => {
		...
		else if (cookieArr.length) $.msg($.name, ``, $.subt, $.subtt);
	})()
		.catch((e) => $.logErr(e))
		.finally(() => $.done())
}
	...
	return new Promise((resolve) => {
		...
		$.post(url, (err, resp, data) => {
			try {
				...
				if (result.status) {
					$.subt += `${$.index}：签到成功;\n`
					$.subtt += `${$.index}：${result.message}\n`
					// $.msg($.name, `签到成功`, `获得${result.data.signinQueryDetailList[0].fragment}个碎片`)
				}
				else {
					$.subt += `${$.index}：签到失败;\n`
					$.subtt += `${$.index}：${result.message}\n`
					// $.msg($.name, `签到失败`, `${result.msg}`)
				}
			} catch (e) {
				$.logErr(e, resp)
			} finally {
...
function getConfig() {
	if ($.isNode()) {
		let cks = [];
		if (process.env.ZYBWJ) {
			if (process.env.ZYBWJ.indexOf('\n') > -1) {
				cks = process.env.ZYBWJ.split('\n');
			} else {
				cks = [process.env.ZYBWJ];
			}
		}
		if (!cks.length) {
			$.log(`\n请先设置Cookie`);
